...
Future initEngine(Map arg) async {
  if (IMEngineManager().engine == null) {
    ...
    if (arg['cloudControlServer'] != null) {
...
Future sendTextMessage(Map arg) async {
  ...
  RCIMIWTextMessage? textMessage = await IMEngineManager().engine
            ...
            onFinished: () {
              RCIWMediaUlits.stopRecordAudio((path, duration) async {
                if (path != null && (duration ?? 0) > 0) {
                  RCIMIWVoiceMessage? msg = await IMEngineManager().engine
                      ?.createVoiceMessage(
                        type,
                        targetId,
                        channelId,
...
Future sendLocationMessage(Map arg) async {
  ...
  BuildContext context = arg['context'];
  RCIWMediaUlits.showImagePicker(context, (XFile? file) async {
    RCIMIWLocationMessage? message = await IMEngineManager().engine
...
Future sendNativeCustomMessage(Map arg) async {
  ...
  RCIMIWNativeCustomMessage? msg = await IMEngineManager().engine
...
sendNativeCustomMediaMessage(Map arg) async {
  ...
  List<File>? files = await RCIWMediaUlits.pickFiles();
  if (files != null) {
    RCIMIWNativeCustomMediaMessage? msg = await IMEngineManager().engine
...
Future sendCustomMessage(Map arg) async {
  ...
  RCIMIWCustomMessage? msg = await IMEngineManager().engine
      ?.createCustomMessage(
        type,
        targetId,
        channelId,
...
Future insertMessages(Map arg) async {
  ...
  RCIMIWTextMessage? textMessage1 = await IMEngineManager().engine
      ?.createTextMessage(
        conversationType,
        targetId,
        channelId,
...
Future modifyUltraGroupMessage(Map arg) async {
  ...
  RCIMIWMessage? textMessage = await IMEngineManager().engine
      ?.createTextMessage(
...
Future<RCIMIWMessage?> _getMessageById(int messageId) async {
  ...
  int code =
      await IMEngineManager().engine?.getMessageById(
        messageId,
        callback: callback,
      ) ??
      -1;
...
Future<RCIMIWMessage?> _getMessageByUId(String messageUId) async {
  ...
  int code =
      await IMEngineManager().engine?.getMessageByUId(
        messageUId,
        callback: callback,
      ) ??
      -1;
...
translateMessagesWithParams(Map arg) async {
  if (arg['messageUid'] == null) {
    RCIWToast.showToast("messageUid 为空");
    return;
  }
  ...
  List<String> sourceLanguages = (arg['sourceLanguage'] ?? '').toString().split(
    ',',
  );
  List<String> targetLanguages = (arg['targetLanguage'] ?? '').toString().split(
    ',',
  );
  ...
  if (useCallback == 1) {
    callback = IRCIMIWTranslateResponseCallback(
      onTranslateResponse: (int? code) {
        ...
        Map<String, String> arg = {};
        arg["listener"] = "translateMessagesWithParams-onTranslateResponse";
        arg["timestamp"] = timeStr;
        arg["code"] = code.toString();
  ...
  int? code = await IMEngineManager().engine?.translateMessagesWithParams(
    params,
    callback: callback,
  );
  DateTime now = DateTime.now();
  String timeStr =
      "${now.hour.toString().padLeft(2, '0')}时${now.minute.toString().padLeft(2, '0')}分${now.second.toString().padLeft(2, '0')}秒";
  Map<String, String> resultCode = {};
  resultCode["listener"] = "translateMessagesWithParams";
  resultCode["timestamp"] = timeStr;
  resultCode["code"] = (code ?? -1).toString();
...
translateTextsWithParams(Map arg) async {
  if (arg['text'] == null) {
    RCIWToast.showToast("text 为空");
    return;
  }
    ...
    callback = IRCIMIWTranslateResponseCallback(
      onTranslateResponse: (int? code) {
        ...
        String timeStr =
            "${now.hour.toString().padLeft(2, '0')}时${now.minute.toString().padLeft(2, '0')}分${now.second.toString().padLeft(2, '0')}秒";
        Map<String, String> arg = {};
        arg["listener"] = "translateTextsWithParams-onTranslateResponse";
        arg["timestamp"] = timeStr;
        arg["code"] = code.toString();
  ...
  int? code = await IMEngineManager().engine?.translateTextsWithParams(
    params,
    callback: callback,
  );
  DateTime now = DateTime.now();
  String timeStr =
      "${now.hour.toString().padLeft(2, '0')}时${now.minute.toString().padLeft(2, '0')}分${now.second.toString().padLeft(2, '0')}秒";
  Map<String, String> resultCode = {};
  resultCode["listener"] = "translateTextsWithParams";
  resultCode["timestamp"] = timeStr;
  resultCode["code"] = (code ?? -1).toString();
...
batchSetConversationTranslateStrategy(Map arg) async {
  ...
  if (arg['strategy'] != "0" &&
      arg['strategy'] != "1" &&
      arg['strategy'] != "2") {
    RCIWToast.showToast("strategy 为非法值");
    return;
  }
  ...
  if (types.length != targetIds.length) {
    RCIWToast.showToast("types 和 targetIds 长度不一致");
    return;
  }

  RCIMIWTranslateStrategy strategy =
      RCIMIWTranslateStrategy.values[int.parse(arg['strategy'])];
  IRCIMIWTranslateResponseCallback? callback;
  if (useCallback == 1) {
    callback = IRCIMIWTranslateResponseCallback(
      onTranslateResponse: (int? code) {
        ...
        Map<String, String> arg = {};
        arg["listener"] =
            "batchSetConversationTranslateStrategy-onTranslateResponse";
        arg["timestamp"] = timeStr;
        arg["code"] = code.toString();
...
updateMyUserProfile(Map arg) async {
  ...
  String birthday = arg['birthday'] ?? "";
  int gender = int.parse(arg['gender'] ?? "0");
  if (gender < 0 || gender > 2) {
    RCIWToast.showToast("gender 为非法值");
    return;
  }
  RCIMIWUserGender genderEnum = RCIMIWUserGender.values[gender];
  String location = arg['location'] ?? "";
  int role = int.parse(arg['role'] ?? "0");
  int level = int.parse(arg['level'] ?? "0");
  String extProfile = arg['userExtProfile'] ?? "{}";

  // 将 JSON 字符串解析为 Map
  Map<String, dynamic> userExtProfileMap = {};
  try {
    userExtProfileMap = jsonDecode(extProfile);
  } catch (e) {
    RCIWToast.showToast("userExtProfileMap 解析失败，请检查格式");
    return;
  }

  RCIMIWUserProfile profile = RCIMIWUserProfile.create(
    userId: userId,
    name: name,
    portraitUri: portraitUri,
    email: email,
    ...
    role: role,
    level: level,
    userExtProfile: userExtProfileMap,
  );
  ...
  int? code = await IMEngineManager().engine?.updateMyUserProfile(
    profile,
    callback: callback,
  );
  DateTime now = DateTime.now();
...
subscribeEvent(Map arg) async {
  ...
  RCIMIWSubscribeType subscribeType =
      RCIMIWSubscribeType.values[int.parse(arg['subscribeType'] ?? "0")];
  int expiry = int.parse(arg['expiry'] ?? "0");
  List<String> userIds = (arg['userIds'] ?? "").split(",");

  RCIMIWSubscribeEventRequest request = RCIMIWSubscribeEventRequest.create(
  ...
  int? code = await IMEngineManager().engine?.subscribeEvent(
    request,
    callback: callback,
  );
  DateTime now = DateTime.now();
...
unSubscribeEvent(Map arg) async {
  ...
  int? code = await IMEngineManager().engine?.unSubscribeEvent(
    request,
    callback: callback,
  );
  DateTime now = DateTime.now();
...
querySubscribeEvent(Map arg) async {
  ...
  int? code = await IMEngineManager().engine?.querySubscribeEvent(
    request,
    callback: callback,
  );
  DateTime now = DateTime.now();
...
querySubscribeEventByPage(Map arg) async {
  int useCallback = int.parse(arg['use_cb'] ?? "1");
  int pageSize = int.parse(arg['pageSize'] ?? "0");
  int startIndex = int.parse(arg['startIndex'] ?? "0");
  RCIMIWSubscribeType subscribeType =
      RCIMIWSubscribeType.values[int.parse(arg['subscribeType'] ?? "0")];
  int expiry = int.parse(arg['expiry'] ?? "0");
  List<String> userIds = (arg['userIds'] ?? "").split(",");
